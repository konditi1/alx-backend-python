    ...
    class Meta:
        ...
        constraints = [
            ...
            models.CheckConstraint(
                check=models.Q(role__in=['admin', 'host', 'guest']),
                name='role_in_choices'
            )
        ]
...
class Message(models.Model):
    ...
    class Meta:
        ordering = ['-sent_at']
        indexes = [
            models.Index(fields=['sender_id'], name='idx_sender_id'),
            models.Index(fields=['recipient_id'], name='idx_recipient_id')
        ]
...
class Conversation(models.Model):
    ...
    def __str__(self):
        return f"{self.participants_id} {self.messages_id} {self.created_at}"


class PaymentMethod(TimeStampedModel):
    pay_method_id = models.UUIDField(
        primary_key=True, 
        default=uuid.uuid4, 
        editable=False
    )
    ...
    currency_conversion = models.DecimalField(
        max_digits=5, 
        decimal_places=2, 
        null=False, 
        default=1.00
...
class Booking(TimeStampedModel):
    ...
    payment_status = models.CharField(
        max_length=12,
        choices=[("in_progress", "In Progress"), ("paid", "Paid"), ("failed", "Failed")],
        default="in_progress"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["property_id"], name="idx_property_bookings"),
            models.Index(fields=["user_id"], name="idx_user_bookings")
        ]
...
class BookingCancellation(models.Model):
    cancellation_id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False
        )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(fields=["booking_id"], name="idx_booking_cancellations")
        ]
...
class Payment(TimeStampedModel):
    ...
    pay_method_id = models.ForeignKey(
        PaymentMethod,
        to_field="pay_method_id",
        on_delete=models.CASCADE,
        related_name="payments"
    )
    ...
    class Meta:
        db_table = "Payment"
        indexes = [
            models.Index(fields=["booking_id"], name="idx_booking_payments"),
            models.Index(field=["status"], name="idx_payment_status")
        ]
...
class Coupon(TimeStampedModel):
    ...
    class Meta:
        db_table = "Coupon"
        indexes = [
            models.Index(fields=["user_id"], name="idx_user_coupons"),
            models.Index(fields=["property_id"], name="idx_property_coupons")
        ]


class CouponUsage(models.Model):
    coupon_usage_id = models.UUIDField(
        primary_key=True,
        default=uuid.uuid4,
        editable=False)
